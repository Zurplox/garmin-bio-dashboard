...
| Unit tests | `python -m unittest discover -s tests -t .` — 254 pass |
...
3. **No static claim beside a live number — and no badge that only says "fine".**
   A tier badge baked to `ATHLETIC`, a colour hardcoded next to a word, and a
   header `LIVE` pill all shipped once. The pill is gone (#38): provenance is
   disclosed in one voice, so the chip states every non-nominal case — stale,
   estimated, or a vault carrying no provenance block — and the nominal header
   carries no badge at all. Derive both from the payload.
...
| #38 | The header's `LIVE` pill is gone, and the provenance chip owns every state that is not nominal. The pill (`headerDataStatePill`, a span beside the wordmark) said what the chip says — `LIVE`, `N ESTIMATED`, `STALE`, `UNVERIFIED` — and it existed only because the chip hid itself in two of those states: measured-and-fresh, and a vault carrying no provenance block at all. Measured through `renderDashboard` with the pill in place: the nominal header read `LIVE`, the estimated header said `1 ESTIMATED` twice over, and a vault with no quality block read `UNVERIFIED` on the pill while the chip stayed hidden — so the one state with no other marker sat on the element a reader cannot act on. Now the chip hides only when the data is measured, fresh *and* verifiable (`if (quality && allLive && !isStale)`), and a vault that published no provenance block is stated on the chip itself — `○ UNVERIFIED` in a neutral tone, opening a panel that says none of the numbers below can be confirmed as measured — rather than borrowing the measured colour. Measured after: the nominal header carries no badge at all, one estimated group states it once (`○ 1 ESTIMATED`, panel opening 18 metric rows), a 40-hour-old sync is marked by its own `STALE DATA` flag plus the chip, and the no-quality vault is stated and openable on the chip with `aria-expanded="true"`. The controls, their hotkeys, the arrival, the hover reservations, the battery and the dashed-line labels are untouched |
